...
#import "BookManagerMainHeaderView.h"

#import "BookManagerMainHeaderViewButton.h"

@interface BookManagerMainHeaderView ()<UIScrollViewDelegate>
...
@implementation BookManagerMainHeaderView
...
- (void)setupUI {
    ...
    for (int i = 0; i < self.titles.count; ++ i) {
        ...
        BookManagerMainHeaderViewButton *titleButton = [[BookManagerMainHeaderViewButton alloc] initWithFrame:frame title:title];
...
- (void)titleButtonClickAction:(BookManagerMainHeaderViewButton *)button {
    
    [button refreshIconImage];
    // deal current button state and other button state
    for (UIButton *btn in self.buttonArray) {
        if ([btn isKindOfClass:[BookManagerMainHeaderViewButton class]]) {
            if (btn.tag != button.tag) {
                BookManagerMainHeaderViewButton *headerButton = (BookManagerMainHeaderViewButton *)btn;
